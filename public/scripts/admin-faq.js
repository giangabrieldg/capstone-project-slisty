document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadFaqs() {
    try {
      ...
      if (faqs.length === 0) {
        faqsTableBody.innerHTML = '<tr><td colspan="3" class="text-center">No FAQs found.</td></tr>';
        return;
      }
      ...
      faqsTableBody.innerHTML = '<tr><td colspan="3" class="text-center text-danger">Unable to load FAQs.</td></tr>';
    }
  }
  ...
  faqsTableBody.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('remove-faq')) {
      if (confirm(`Are you sure you want to delete the FAQ: "${e.target.dataset.question}"?`)) {
  ...
  async function deleteFaq(question) {
    try {
      const response = await fetch(`/api/faqs/${encodeURIComponent(question)}`, {
        method: 'DELETE',
      });
  ...
  faqForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      const url = editQuestion ? `/api/faqs/${encodeURIComponent(editQuestion)}` : '/api/faqs';
