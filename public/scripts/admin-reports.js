// Initialize charts
      ...
      function updateSummaryCards(summary) {
        const cards = [
          ...
          {
            id: "avgOrderValue",
            value: `₱${(parseFloat(summary?.average_order_value) || 0).toFixed(
              2
            )}`,
          },
        ];
        ...
        orders.forEach((order) => {
          ...
          const items = order.items
            .map((item) => {
              ...
              return `${item.name} (Qty: ${quantity}) - PHP ${(
                price * quantity
              ).toFixed(2)}`;
            })
            .join("<br>");
          ...
          // Create customer details with payment and delivery methods
    ...
    `;
          ...
          row.innerHTML = `
    <td>#${order.orderId}</td>  <!-- This will now show ORD001 format -->
...
`;
    ...
    Generated on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}
        ...
        tbody.insertBefore(summaryRow, tbody.lastChild);
