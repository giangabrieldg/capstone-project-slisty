document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadInquiries() {
    try {
      ...
      inquiriesTableBody.innerHTML =
        '<tr><td colspan="7">Error loading inquiries.</td></tr>';
    }
  }

  // Function to render inquiries in the table
  function renderInquiries(inquiries) {
    inquiriesTableBody.innerHTML = inquiries
      .map(
        (inquiry) => `
      <tr>
        ...
        <td><span class="status ${inquiry.status.toLowerCase()}">${
          inquiry.status
        }</span></td>
        <td>
          ${
      ...
      button.addEventListener("click", async () => {
        ...
        try {
          const response = await fetch(
            `${window.API_BASE_URL}/api/inquiries/reply`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
  ...
  function filterInquiries() {
    ...
    rows.forEach((row) => {
      ...
      const matchesSearch =
        customer.includes(searchTerm) || subject.includes(searchTerm);
