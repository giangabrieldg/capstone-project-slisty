async function loadCartItems() {
  ...
  if (!token) {
    ...
    cartItemsContainer.innerHTML +=
      '<p><a href="/customer/login.html" class="btn btn-primary">Log In</a></p>';
    return;
  }
  ...
  try {
    ...
    console.log(
      "Cart items response:",
      JSON.stringify(data.cartItems, null, 2)
    );
    ...
    checkoutBtn.disabled = cartItems.some(
      (item) => item.name === "Unknown" || item.price === 0
    );
    ...
    cartItems.forEach((item) => {
      ...
      html += `
        <tr>
          ...
          <td>
            <button class="btn btn-sm btn-danger remove-item" data-cart-item-id="${
              item.cartItemId
            }">Remove</button>
          </td>
        </tr>
      ...
      button.addEventListener("click", async () => {
        ...
        try {
          const response = await fetch(
            `${window.API_BASE_URL}/api/cart/remove`,
            {
              method: "DELETE",
              headers: {
