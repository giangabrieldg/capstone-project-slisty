  ...
  menuItems.forEach((item) => {
    ...
    const stockDisplay =
      item.hasSizes && item.sizes?.length > 0
        ? item.sizes
            .map((size) => `${size.sizeName} - ${size.stock}`)
            .join("<br>")
        : item.stock != null
        ? item.stock
        : "0";
...
function formatPriceDisplay(item) {
  if (item.hasSizes && item.sizes?.length > 0) {
    return item.sizes
      .map((size) => `${size.sizeName} - ₱${Number(size.price).toFixed(2)}`)
      .join("<br>");
  }
...
function showSizeInputs(show) {
  ...
  const sizeInputs = document.querySelectorAll(
    '#sizesContainer input[name^="size_"], #sizesContainer input[name^="price_"], #sizesContainer input[name^="stock_"]'
  );
...
function setLoadingState(isLoading, message = "") {
  ...
  document
    .querySelectorAll(".edit-item, .remove-item, .add-item-btn")
    .forEach((btn) => {
  ...
  if (hasSizes) {
    ...
    for (let index = 0; index < sizeInputs.length; index++) {
      ...
      if (
        ...
        showError(
          "Validation Error",
          new Error("Please enter valid sizes, prices, and stock for all sizes")
        );
        return;
      }
    ...
    if (
      ...
      showError(
        "Validation Error",
        new Error("Please enter valid price and stock")
      );
      return;
    }
