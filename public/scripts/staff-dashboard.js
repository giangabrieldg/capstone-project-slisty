  ...
  try {
    ...
    orders.forEach((order) => {
      ...
      row.innerHTML = `
        ...
        <td>
          <button class="btn btn-sm btn-primary update-status-btn" data-order-id="${
            order.orderID
          }">
            Update Status
          </button>
    ...
    document.querySelectorAll(".update-status-btn").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        const newStatus = prompt(
          "Enter new status (e.g., Processing, Completed):"
        );
        if (newStatus) {
          try {
            const response = await fetch(
              `${window.API_BASE_URL}/api/orders/${orderId}/status`,
              {
                method: "PUT",
                headers: {
